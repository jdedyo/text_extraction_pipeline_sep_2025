...
# IF NB_CPUS > 10, it will submit using --array directive to avoid being killed by the scheduler
...
sbatch_script="download_and_process_pdfs.sbatch"  # Change this to the name of your SBATCH script template; will overwrite any job name in the sbatch file

if [ "$cpus" -gt 10 ]; then
    # submit using --array directive to avoid being killed by the scheduler if more than 50 jobs
    sbatch --array=1-$cpus --job-name="download_and_process_pdfs_$year" "$sbatch_script" $year
else
    ...
    for i in $(seq 1 $cpus);
        ...
        job_name="download_and_process_pdfs_${year}_${i}"  # Change this to your desired job name
